...
class GBD
{
    ...
    public function generaClases(string $basedatos=null)
    {
        foreach ($this->getTablas($basedatos) as $tabla)
        {
            $fichero=fopen("./Clases/".$tabla.".php","wt");
            fwrite($fichero,"<?php\nclass ".$tabla."\n{\n");
            foreach ($this->getColumnas($tabla) as $columna)
            {
                fwrite($fichero,"\tpublic \$".$columna.";\n");
            }
            fwrite($fichero,"}\n");
            fclose($fichero);
        }
    ...
    public function getTablas($basedatos)
    {
        $consulta=$this->conexion->query("show tables");
        $tablas=$consulta->fetchAll(PDO::FETCH_NUM);
        $tablas2=array_column($tablas,0);
        return $tablas2;
    }

    public function getColumnas($tabla)
    {
        $consulta=$this->conexion->query("show columns from $tabla");
        $columnas=$consulta->fetchAll();
        $columnas2=array_column($columnas,"Field");
        return $columnas2;
    }
}
